...
# 2020-01-13
### 메인페이지 디자인 캐러셀로 변경
부트스트랩 3.3.1 -> 4.4.1 로 업그레이드 리엔지니어링<Br>
회원가입 버튼 클릭 시 아이디 비밀번호 요구하는 오류 수정

# 2020-01-14
### 답변형 게시판 제작 중
### 데이터베이스 구조 변경 구상 중
1. 단순히 기존처럼 특정 게시글에 걸려있는 비밀번호를 입력하면 해당 글을 수정 또는 삭제 할 수 있는 것이 아닌, 로그인 한 회원이 자신이 쓴 글에 대해서만 수정하고 삭제할 수 있도록 데이터베이스 구조를 수정하였음.(ER Diagram 수정)<br>
=> jsp_member 테이블과 board 테이블을 1:다 관계로 설정하고 board table 에 jsp_member 테이블의 primary key 인 id 을 foreign key 로 설정.
2. 도서관리 프로그램에서 단순히 도서를 등록하고 수정, 삭제하는 기능은 밋밋하다고 판단함. 게시판과 마찬가지로, 회원과 책이 연관을 맺도록 데이터베이스 구조 수정
=> jsp_member 테이블과 jsp_book 테이블을 다:다 관계로 설정하고 이 둘 사이의 관계를 맺어줄 daechul 이라는 테이블 추가. 새로 만들 이 테이블에는 jsp_member 의 primary key 인 id 과 jsp_book 의 primary key 인 name 을 주키이자 외래키로써 등록.
...
* 아이디 중복확인 기능 추가 예정 -> ajax 이용
* 주민번호 입력란 삭제 예정
* 비밀번호 확인 기능 추가 예정 -> ajax 이용하여 동적으로 비밀번호 확인하도록 
### 게시판 구조 수정중
DB 스키마 수정중
